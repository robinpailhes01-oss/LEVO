...
- **Accueil = le parcours (grammaire scrollcraft « monde continu »)**, brief dans `scrollcraft/builds/luma-parcours/BRIEF.md`, ligne au registre `scrollcraft/FINGERPRINTS.md`. `components/journey/` : `track.ts` (six lieux, stations/trajets, 11,4 vh, fonctions cameraParam/local/presence), `World.tsx` (R3F : caméra sur la piste, comptoir, coulée instanciée + cavité, cœur + satellites + liaisons, atelier, tableau avec vrais KPI Harmonie Yacht, bureau, bulle voyageuse en Html), `Journey.tsx` (stage fixe + spacer, fenêtres de texte par station, champ « votre demande » → bulle → mailto pré-rempli, rail-carte, mobile : visée remontée + voile crème bas, reduced-motion : scroll natif). Popup audit désactivée en auto sur `/` (AuditProvider). Contenu profond déplacé : `/realisations`, `/a-propos` (+ méthode + preuve), `/formations`. Nav et footer pointent vers ces pages. L'ancienne accueil en actes n'existe plus sur `/` (ses sections vivent sur les pages dédiées). **Non fusionné en prod** : à valider par Robin sur la preview.
